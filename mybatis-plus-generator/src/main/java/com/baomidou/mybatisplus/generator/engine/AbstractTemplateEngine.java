...
public abstract class AbstractTemplateEngine {
    ...
    protected void outputService(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        ...
        if (StringUtils.isNotBlank(tableInfo.getServiceImplName()) && StringUtils.isNotBlank(serviceImplPath)) {
            getTemplateFilePath(TemplateConfig::getServiceImpl).ifPresent(serviceImpl -> {
                String replacePath = getReplacePath(serviceImplPath, entityName);
                String controller1 = "ServiceImpl";
    ...
    protected void outputController(@NotNull TableInfo tableInfo, @NotNull Map<String, Object> objectMap) {
        ...
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(controllerPath)) {
            getTemplateFilePath(TemplateConfig::getController).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String replacePath = getReplacePath(controllerPath, entityName);
    ...
    private String getPackageName(@NotNull Map<String, Object> objectMap, String controller1, String entityName) {
        String aPackage = (String) ((Map) objectMap.get("package")).get(controller1);
        return org.apache.commons.lang3.StringUtils
            .replace(aPackage, "module", "module"
                + "." + entityName.toLowerCase(Locale.ROOT));
    }

    protected String getMapperPackageName(Map<String, Object> objectMap) {
        String aPackage = (String) ((Map) objectMap.get("package")).get("Mapper");
        return org.apache.commons.lang3.StringUtils
            .replace(aPackage, "module", "dao");
    }

    protected String getEntityPackageName(Map<String, Object> objectMap) {
        String aPackage = (String) ((Map) objectMap.get("package")).get("Entity");
        return org.apache.commons.lang3.StringUtils
            .replace(aPackage, "module", "dao");
    }


    private void outputRespDto(TableInfo tableInfo, Map<String, Object> objectMap) {
        String respDtoPath = getPathInfo(OutputFile.respDto);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(respDtoPath)) {
            getTemplateFilePath(TemplateConfig::getRespDto).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String clazzName = entityName + "RespDTO";
                objectMap.put("clazzName", clazzName);
                String replacePath = getReplacePath(respDtoPath, entityName);
                String controller1 = "RespDTO";
                String packageName = getPackageName(objectMap, controller1, entityName);
                objectMap.put("respDtoPackageName", packageName);
                String file = String.format((replacePath + File.separator + clazzName + suffixJavaOrKt()), entityName);
                outputFile(new File(file), objectMap, controller, true);
            });
        }
    }

    private void outputPageQueryReqDto(TableInfo tableInfo, Map<String, Object> objectMap) {
        String path = getPathInfo(OutputFile.pageQueryReqDto);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(path)) {
            getTemplateFilePath(TemplateConfig::getPageQueryReqDtp).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String clazzName = entityName + "PageQueryReqDTO";
                objectMap.put("clazzName", clazzName);
                String controller1 = "PageQueryReqDTO";
                String packageName = getPackageName(objectMap, controller1, entityName);
                objectMap.put("pageQueryReqDtoPackageName", packageName);
                String replacePath = getReplacePath(path, entityName);
                String file = String.format((replacePath + File.separator + clazzName + suffixJavaOrKt()), entityName);
                outputFile(new File(file), objectMap, controller, true);
            });
        }
    ...
    private void outputUpdateReqDto(TableInfo tableInfo, Map<String, Object> objectMap) {
        String path = getPathInfo(OutputFile.createReqDto);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(path)) {
            getTemplateFilePath(TemplateConfig::getUpdateReqDto).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String clazzName = entityName + "UpdateReqDTO";
                objectMap.put("clazzName", clazzName);
                String controller1 = "UpdateReqDTO";
                String packageName = getPackageName(objectMap, controller1, entityName);
                objectMap.put("updateReqDtoPackageName", packageName);
                String replacePath = getReplacePath(path, entityName);
                String file = String.format((replacePath + File.separator + clazzName + suffixJavaOrKt()), entityName);
                outputFile(new File(file), objectMap, controller, true);
            });
    ...
    private void outputCreateReqDto(TableInfo tableInfo, Map<String, Object> objectMap) {
        String path = getPathInfo(OutputFile.createReqDto);
        if (StringUtils.isNotBlank(tableInfo.getControllerName()) && StringUtils.isNotBlank(path)) {
            getTemplateFilePath(TemplateConfig::getCreateReqDto).ifPresent(controller -> {
                String entityName = tableInfo.getEntityName();
                String clazzName = entityName + "CreateReqDTO";
                objectMap.put("clazzName", clazzName);
                String controller1 = "CreateReqDTO";
                String packageName = getPackageName(objectMap, controller1, entityName);
                objectMap.put("createReqDtoPackageName", packageName);
                String replacePath = getReplacePath(path, entityName);
                String file = String.format((replacePath + File.separator + clazzName + suffixJavaOrKt()), entityName);
                outputFile(new File(file), objectMap, controller, true);
            });
    ...
    private String getReplacePath(String respDtoPath, String entityName) {
        return org.apache.commons.lang3.StringUtils
            .replace(respDtoPath, "module", "module"
                + File.separator + entityName.toLowerCase(Locale.ROOT));
    }
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            tableInfoList.forEach(tableInfo -> {
                ...
                // entity
                if (objectMap.get("disabledEntity") == null || objectMap.get("disabledEntity").equals(false)) {
                    outputEntity(tableInfo, objectMap);
                }
                // mapper and xml
                outputMapper(tableInfo, objectMap);

                if (objectMap.get("disabledDto") == null || objectMap.get("disabledDto").equals(false)) {
                    outputCreateReqDto(tableInfo, objectMap);
                    outputUpdateReqDto(tableInfo, objectMap);
